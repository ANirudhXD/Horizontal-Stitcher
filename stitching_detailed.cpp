// save point 2: removed scene related frame selection that was based on pair-wise key-point matches.
...
int main(int argc, char* argv[])
{
    ...
    {
        // Add the path of each file to the vector
    ...
    vector<int> indices;
    for(int i=1;i<=num_images;i++) indices.push_back(i);
    ...
    for (size_t i = 0; i < cameras.size(); i++)
    {
        ...
        cameras[i].R = R;
        LOGLN("Initial camera intrinsics #" << indices[i] << ":\nK:\n" << cameras[i].K() << "\nR:\n" << cameras[i].R);
    }
